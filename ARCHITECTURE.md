...
| [13] Stakeholder Adaptation | `src/mve_generator.py::derive_role_view` + `module5_responses/module5_pipeline.py::render_views_for_alert` | Per-role views implemented (closes GAP-A2): `OperatorRole` enum (IT_generalist / biomed_engineer / nurse_manager); `derive_role_view(mve, role)` returns role-scoped MVEOutput. Layer 2 is invariant across roles (INVARIANT 6 cross-role severity consistency); Layer 3 `clinical_constraint` (DO NOT wording — INVARIANT 7) is preserved across roles; Layer 3 `immediate_action` is rewritten to role-appropriate verbs. Notification routing via `module5_pipeline.RESPONSE_POLICY.routing_overrides` remains the primary/secondary delivery channel. Covered by 8 tests in `tests/test_safe_failure.py`. |
