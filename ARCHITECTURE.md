...
3. **Module 2 - Detection Training** (`module2_detection/`)
   ...
   - **Track A: XGBoost-only production classifier**. Random Forest and Decision Tree are retained as **comparative baselines** (reported in thesis Section 4) but excluded from the production inference path. Selection rationale: comparative evaluation showed XGBoost dominates on F2 and AUC (0.9941); maxing three correlated tree models inflated FPR without measurable FNR benefit.
   - **Track B: a denoising autoencoder (DAE) trained on benign-only behavior, raw features only.** Earlier cascade design `[raw || P_xgb, P_rf, P_dt]` was evaluated via leave-one-class-out ablation on EHMS-2020 (N=2 folds, ΔAUC = +0.02 marginal) and MedSec-25 (N=4 folds, ΔAUC = −0.19 regression). Cascade fails systematically on higher-dimensional inputs due to capacity dilution; **production design uses DAE-raw (25 features only)**. Cascade is reported as a negative ablation result.
   - Trained artifacts under `results/models/`. Module 2 enforces leakage assertions: training functions raise `RuntimeError` if `test_phase1.parquet` or `demo_phase1.parquet` is loaded.
...
│  │                                                                          │  │
│  │   Configuration: config/composite_risk_weights.yaml                     │  │
...
| --- | --- | --- |
| [1] Data Preparation | `module1_preprocessing/phase1/pipeline.py` | **4-way stratified split** (Strategy 1 — Frozen Test + Demo Pool): train (60%), val (15%), test (15% frozen), demo (10% frozen). SMOTE applied to train only. Class proportions preserved within ±2% (verified). Provenance in `data/processed/split_metadata.yaml`. Random state 42. |
| [2] Track A Training | `module2_detection/module2_train_models.py` (`train_track_a`) | **XGBoost-only production**. RF and DT trained as comparative baselines (thesis Section 4) but excluded from inference path. Selection rationale: XGBoost dominates F1/AUC (0.9941); `max(P_xgb, P_rf, P_dt)` inflated FPR without measurable FNR benefit. Leakage assertions raise `RuntimeError` if test/demo splits are loaded. |
| [3] Track B Training | `module2_detection/module2_train_models.py::train_track_b_dae` | **DAE on raw 25 features only** (no cascade). Cascade design `[raw \|\| P_xgb, P_rf, P_dt]` evaluated via leave-one-class-out on EHMS-2020 (N=2, ΔAUC=+0.02 marginal) and MedSec-25 (N=4, ΔAUC=−0.19 regression). Dropped from production. Negative ablation result reported in thesis. Trained on `benign_only_train.parquet` (derived from train split only). |
...
- XGBoost dominates on F1 and AUC (0.9941)
...
#### `config/composite_risk_weights.yaml` (Step 9)
