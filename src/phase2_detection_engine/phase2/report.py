...
def render_detection_report(report: Dict[str, Any]) -> str:
    ...
    w("")
    w("This section describes the CNN→BiLSTM→Attention feature extraction "
    ...
    w("")
    w("1. **CNN (Convolutional Neural Network):** Extracts local spatial "
      "patterns from sliding windows of consecutive network events.")
    w("2. **BiLSTM (Bidirectional Long Short-Term Memory):** Captures "
      "temporal dependencies in both forward and backward directions.")
    w("3. **Bahdanau Attention:** Computes adaptive weights over timesteps "
    ...
    w("  │")
    w("  ├─── [CNN Block]")
    w(f"  │      Conv1D({cnn_filters[0]}, k={hp.get('cnn_kernel_size', 3)}, "
      f"{hp.get('cnn_activation', 'relu')}) → MaxPool({cnn_pool_size})")
    w(f"  │      Conv1D({cnn_filters[1]}, k={hp.get('cnn_kernel_size', 3)}, "
      f"{hp.get('cnn_activation', 'relu')}) → MaxPool({cnn_pool_size})")
    w(f"  │      Output: (batch, {cnn_timesteps}, {cnn_filters[1]})")
    w("  │")
    w("  ├─── [BiLSTM Block]")
    w(f"  │      Bidirectional LSTM({bilstm_units[0]}, return_seq=True) → "
      f"Dropout({hp.get('dropout_rate', 0.3)})")
    w(f"  │      Bidirectional LSTM({bilstm_units[1]}, return_seq=True) → "
      f"Dropout({hp.get('dropout_rate', 0.3)})")
    w(f"  │      Output: (batch, {cnn_timesteps}, {bilstm_out_dim})")
    w("  │")
    w("  └─── [Attention Block]")
    w(f"         Dense({hp.get('attention_units', 128)}, tanh) → Dense(1, softmax) "
    ...
    w("")
    w(f"Network traffic samples are grouped into temporal windows of "
    ...
    w("")
    w("The Bahdanau (additive) attention mechanism computes a context "
      "vector by learning to weight each timestep according to its "
      "relevance for intrusion detection:")
    w("")
    w(f"1. **Score:** `Dense({hp.get('attention_units', 128)}, tanh)` projects "
      f"each timestep to a score vector")
    w("2. **Weight:** `Dense(1)` + `softmax(axis=timesteps)` normalises "
      "scores to a probability distribution over timesteps")
    w("3. **Multiply:** Element-wise multiplication weights the BiLSTM "
      "output sequence by learned attention weights")
    w(f"4. **Pool:** `GlobalAveragePooling1D` produces the final "
    ...
    w("|----------------|-------|-----------|")
    w(f"| Timesteps | {hp.get('timesteps', 20)} | "
      f"Captures short-term traffic patterns |")
    w(f"| Stride | {hp.get('stride', 1)} | "
      f"Maximum window overlap for data coverage |")
    w(f"| CNN filters | [{cnn_filters[0]}, {cnn_filters[1]}] | "
      f"Hierarchical spatial feature extraction |")
    w(f"| CNN kernel size | {hp.get('cnn_kernel_size', 3)} | "
      f"3-sample receptive field |")
    w(f"| CNN activation | {hp.get('cnn_activation', 'relu')} | "
      f"Standard non-linearity |")
    w(f"| BiLSTM units | [{bilstm_units[0]}, {bilstm_units[1]}] | "
      f"Forward + backward temporal context |")
    w(f"| Dropout rate | {hp.get('dropout_rate', 0.3)} | "
      f"Regularisation against overfitting |")
    w(f"| Attention units | {hp.get('attention_units', 128)} | "
      f"Score vector dimensionality |")
    w(f"| Random state | {hp.get('random_state', 42)} | "
      f"Reproducibility |")
    w("")
    ...
    w("|----------|-------------|")
    w(f"| `detection_model.weights.h5` | Model weights "
      f"({report.get('total_parameters', 0):,} parameters, "
      f"no classification head) |")
    w(f"| `attention_output.parquet` | Weighted sum vectors "
      f"({report.get('output_dim', 128)}-dim) + labels + split indicator |")
    w("| `detection_report.json` | Model summary, layer shapes, "
    ...
    w("| Metric | Value |")
    w("|--------|-------|")
    w(f"| Total execution time | "
      f"**{report.get('elapsed_seconds', 0):.2f} s** |")
