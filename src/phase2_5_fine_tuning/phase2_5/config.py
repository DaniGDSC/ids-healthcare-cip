...
class SearchSpaceConfig(BaseModel):
    ...
    cnn_filters_1: List[int] = [32, 64, 128]
    cnn_filters_2: List[int] = [64, 128, 256]
    cnn_kernel_size: List[int] = [3, 5]
    bilstm_units_1: List[int] = [64, 128, 256]
    bilstm_units_2: List[int] = [32, 64, 128]
    dropout_rate: Union[List[float], ContinuousRange] = ContinuousRange(low=0.05, high=0.5, log=False)
    attention_units: List[int] = [64, 128, 256]
    timesteps: List[int] = [10, 20, 30]
    batch_size: List[int] = [128, 256, 512]
    learning_rate: Union[List[float], ContinuousRange] = ContinuousRange(low=0.00005, high=0.005, log=True)
...
class Phase2_5Config(BaseModel):
    ...
    def _valid_metric(cls, v: str) -> str:
        allowed = {"f1_score", "auc_roc", "accuracy", "precision", "recall"}
        if v not in allowed:
