...
class TestDataLoaderConfigIntegration:
    """Test DataLoader.from_config integration."""
    
    def test_dataloader_from_config(self, tmp_path):
        """Verify DataLoader.from_config creates instance with config."""
        from src.phase1_preprocessing.data_loader import DataLoader
        ...
        config = {
            "max_file_size_mb": 20480,
            "max_total_size_mb": 100000,
            "io_workers": 8,
            "skip_on_error": True,
        }
        
        loader = DataLoader.from_config(str(data_dir), config)
        
        # Verify configuration was applied
        assert loader.max_file_size_mb == 20480
        assert loader.max_total_size_mb == 100000
        assert loader.skip_on_error is True
        assert loader.data_dir == data_dir
    
    def test_dataloader_from_config_with_clamping(self, tmp_path):
        """Verify from_config clamps values to bounds."""
        from src.phase1_preprocessing.data_loader import DataLoader
        ...
        config = {
            "max_file_size_mb": 500_000,  # Exceeds max
            "io_retries": -1,             # Below min
        }
        
        loader = DataLoader.from_config(str(data_dir), config)
        
        # Should be clamped
        assert loader.max_file_size_mb == 204800  # Max bound
        assert loader.io_retries >= 0             # Min bound
    
    def test_dataloader_from_config_uses_defaults(self, tmp_path):
        """Verify from_config uses defaults for missing keys."""
        from src.phase1_preprocessing.data_loader import DataLoader
        ...
        assert loader.max_file_size_mb == DEFAULT_LOADER_CONFIG["max_file_size_mb"]
        assert loader.io_retries == DEFAULT_LOADER_CONFIG["io_retries"]
        assert loader.skip_on_error == DEFAULT_LOADER_CONFIG["skip_on_error"]
