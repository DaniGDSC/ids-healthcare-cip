...
def load_dataset(path: Path) -> pd.DataFrame:
    ...
    cfg = Phase0Config.from_yaml(PROJECT_ROOT / "pipeline/module0_analysis/phase0/config.yaml")
...
def compute_class_distribution(
    ...
    dist["imbalance_ratio"] = (
        round(majority / minority, DECIMAL_PLACES) if minority else float("inf")
    )
...
def top_variance_features(
    ...
    top = [
        (str(name), round(float(val), DECIMAL_PLACES)) for name, val in variances.head(k).items()
    ]
...
def generate_report(
    ...
    w("")
    w(
        f"The experiments in this study employ the WUSTL-EHMS-2020 dataset [WUSTL-EHMS-2020], "
    ...
    w("")
    w(
        f"A stratified {TRAIN_RATIO:.0%}/{TEST_RATIO:.0%} train/test split preserves "
    ...
    w("")
    w(
        f"The imbalance ratio is **{ratio:.{DECIMAL_PLACES}f}:1** (Normal : Attack). "
    ...
    w("")
    w(
        f"Table 2 lists the top {TOP_VARIANCE_K} features ranked by sample variance, "
    ...
    w("")
    w(
        f"This confirms that network-flow volume features (byte counts, packet counts) "
    ...
    if missing:
        ...
        w("")
        w(
            "This demonstrates that the dataset contains a limited number of missing "
        ...
        w("The dataset contains **zero missing values** across all " f"{n_cols} attributes.")
        w("")
        w(
            "This confirms that the WUSTL-EHMS-2020 dataset is acquisition-complete, "
    ...
    w("")
    w(
        f"Pearson correlation analysis identifies **{len(high_pairs)}** feature pairs "
    ...
    w("")
    w(
        "This demonstrates the presence of redundant feature pairs that inflate "
    ...
    w("")
    w(
        f"Descriptive statistics (mean, median, standard deviation, min, max) were "
    ...
    w("")
    w(
        "- Biometric features exhibit low variance and narrow physiological ranges, "
        "consistent with stable patient vital signs under normal operating conditions."
    )
    w(
        "- Network-traffic features span multiple orders of magnitude, motivating "
