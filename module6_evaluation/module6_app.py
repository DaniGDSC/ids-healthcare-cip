...
def render_mve_layers(alert: dict) -> None:
    ...
    ARCHITECTURE.md Step [13]/[11]/[12] affordances: emits the shared
    anchor block (Invariant 9), SHAP stability indicator (Step [11]),
    and Mode A audit chip (Step [12]) above the Layer 1/2/3 content.
    Each helper is no-op when the alert lacks the corresponding fields,
    so legacy alerts still render cleanly.
    """
    # Affordances first — render at the TOP of the alert panel.
    render_shared_anchor(alert)
    render_shap_stability_indicator(alert)
    render_llm_audit_chip(alert)
...
# ── ARCHITECTURE.md Step [13] / [14] dashboard affordances ────────────


def render_shared_anchor(alert: dict) -> None:
    """ARCHITECTURE.md Step [13] / INVARIANT 9 — shared header.

    Identical 5-field block (alert_id, risk_tier, device_id,
    one-line summary, timestamp) rendered above the role-specific
    Layer 1 content. Prevents miscommunication during phone-based
    incident handling — every operator sees the same anchor regardless
    of role.

    No-op when the alert has no ``shared_anchor`` block (legacy alerts
    pre-Phase-M6-fix).
    """
    anchor = alert.get("shared_anchor") or {}
    if not anchor:
        return
    aid = anchor.get("alert_id") or alert.get("alert_id", "—")
    rt  = anchor.get("risk_tier") or alert.get("risk_level", "—")
    dev = anchor.get("device_id") or alert.get("device_class", "—")
    summary = anchor.get("one_line_summary") or ""
    ts = anchor.get("timestamp") or ""
    # Clean horizontal block — left-justified, monospaced for
    # phone-handoff readability.
    st.info(
        f"**Alert** `{aid}` · **Tier** `{rt}` · **Device** `{dev}`  \n"
        f"*{summary}*  \n"
        f"Timestamp: `{ts}`"
    )


def render_shap_stability_indicator(alert: dict) -> None:
    """ARCHITECTURE.md Step [11] — SHAP stability indicator.

    Surfaces a fragility warning when ``stability_score < 0.90`` per
    the doc cutoff. Silent when stability is high (the common case).
    Also flags ``xgboost_low_confidence`` for NOVEL_ANOMALY alerts
    (DAE drove the alert; XGB SHAP is computed but not faithful).
    """
    stability = float(alert.get("shap_stability_score", 1.0))
    is_stable = bool(alert.get("shap_is_stable", stability >= 0.90))
    source = str(alert.get("shap_source", "xgboost"))

    if not is_stable:
        st.warning(
            f"⚠️ **SHAP fragility warning** — stability score "
            f"`{stability:.2f}` (< 0.90 threshold). Top-3 features "
            "may shift under small input perturbations; treat the "
            "narrative as suggestive, not load-bearing."
        )
    if source == "xgboost_low_confidence":
        st.warning(
            "⚠️ **SHAP source: low confidence** — this alert was "
            "driven by the DAE (NOVEL_ANOMALY); XGBoost SHAP is "
            "computed but not faithful. Per-feature DAE "
            "reconstruction-error attribution is future work."
        )


def render_llm_audit_chip(alert: dict) -> None:
    """Render a small chip showing the LLM provider + model_version
    when the alert was generated by Mode A. Mode B emits no chip.

    Defense-week: lets an examiner verify, at a glance, that Mode A
    metadata is being persisted on each curated alert (audit-trail
    visibility per ARCHITECTURE.md Step [12]/Step [16])."""
    if alert.get("mve_mode") != "A_llm":
        return
    provider = alert.get("llm_provider") or "unknown"
    model = alert.get("llm_model_version") or "unknown"
    st.caption(
        f"🛈 Mode A — provider: `{provider}` · model: `{model}` "
        "(full prompt + response in audit log)"
    )
