...
def build_analyst_report(
    ...
    # Phase B: only iterate Track A models whose predictions are present
    # (production runtime is XGB-only; RF/DT are skipped when pickles absent).
    available_track_a = [n for n in TRACK_A_MODELS if n in all_preds]
    pred_matrix = np.column_stack(
        [all_preds[name]["y_pred"] for name in available_track_a]
        + [dae_preds["y_pred"]]
    )  # (N, n_available + 1)
...
def build_clinician_summaries(
    ...
    # Phase B: only available Track A pipelines contribute (XGB-only by default).
    available_track_a = [n for n in TRACK_A_MODELS if n in all_preds]
    pred_matrix = np.column_stack(
        [all_preds[name]["y_pred"] for name in available_track_a]
        + [dae_preds["y_pred"]]
...
def validate_consistency(
    ...
    for name, cfg in TRACK_A_MODELS.items():
        # Phase B: skip baselines whose pickles aren't present.
        pipeline_path = PROJECT_ROOT / cfg["pipeline"]
        if not pipeline_path.exists() or name not in all_shap:
            continue
        obj = loads_signed(pipeline_path)
        clf = obj.named_steps["classifier"] if hasattr(obj, "named_steps") else obj
...
def validate_perturbation(
    ...
    for name in TRACK_A_MODELS:
        # Phase B: skip baselines whose pickles aren't loaded.
...
def main() -> None:
    ...
    for name, cfg in TRACK_A_MODELS.items():
        # Phase B: production runtime is XGB-only. Skip RF/DT SHAP when
        # their pickles are absent (re-train via
        # ``module2_train_models.py --include-baselines`` for thesis
        # Section 4 reproducibility).
        if not (PROJECT_ROOT / cfg["pipeline"]).exists():
            logger.info(
                "  %s pipeline absent — skipping SHAP "
    ...
    sq_err, weighted_err, feat_weights = compute_dae_feature_errors(
        dae_json, dae_weights, X_test, feat_names,
    )
