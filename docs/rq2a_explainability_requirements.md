...
| 5 | **Audience appropriateness** — different roles see different framings | Three role renderers in `module6_evaluation/module6_app.py` — `render_analyst` (IT Generalist view), `render_clinician` (Nurse Manager view), `render_admin` (Biomed Engineer view) — plus a 4-th channel via `render_mve_layers` collapsible expander. Canonical role display names defined in `ROLE_DISPLAY_NAMES` (spec triad). | RQ3 §1 capability table (cross-role consistency tests) | ✓ |
