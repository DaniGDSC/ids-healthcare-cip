...
- **Role switcher:** `st.sidebar.pills("View as", ROLE_DISPLAY_LIST, key="sim_role")` in `simulation_mode()` (Online Simulation page). Spec-triad display labels (IT Generalist / Biomed Engineer / Nurse Manager) dispatched to three render functions on internal key: `render_analyst` (IT Generalist view), `render_admin` (Biomed Engineer view), `render_clinician` (Nurse Manager view). The Dashboard page uses a compact pill variant (`ROLE_SHORT_LABELS` — IT / Biomed / Nurse) in `_triage_actions_column`. Canonical name + key mapping in `ROLE_DISPLAY_NAMES` / `ROLE_INTERNAL_KEY` near the top of `module6_app.py`. The prototype's role toggle (`sentinel_dashboard.html:883-884`) exposes only SOC / Clinical — the third role (Biomed Engineer / Administrator) is absent from the prototype HTML. *(Session 13 Q-W4; revised after spec-triad role unification)*
...
Current implementation (post spec-triad rename): `st.sidebar.pills("View as", ROLE_DISPLAY_LIST, ...)` in `simulation_mode()` — display labels are the spec triad **IT Generalist / Biomed Engineer / Nurse Manager** (see `ROLE_DISPLAY_NAMES` const). Render dispatch via `ROLE_INTERNAL_KEY` mapping → if/elif on internal key (`analyst` / `administrator` / `clinician`) → `render_analyst` (IT Generalist view) / `render_admin` (Biomed Engineer view) / `render_clinician` (Nurse Manager view). The prototype's role toggle at `sentinel_dashboard.html:879-933` exposes only two positions (SOC / Clinical) via a pill-style toggle inside the MVE card; the third spec-triad role (Biomed Engineer) has no prototype representation. Action vocabulary mismatch: current `ACTIONS = ["dismiss", "monitor", "investigate", "isolate", "escalate"]` (L55) vs prototype's `Acknowledge / Escalate / Dismiss` (L939-950) — three of five current actions are absent from prototype; prototype's "Acknowledge" has no exact match in current ACTIONS. *(Session 13 Q-W4, Q-W5; revised after spec-triad rename)*
